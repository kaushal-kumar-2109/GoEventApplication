...
// helper used when creating users: SHA256 then bcrypt
const sha256 = async (value) => {
    return await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, value);
};

// verify a plain password against stored bcrypt hash (which was bcrypt(sha256(pass)))
const verifyPassword = async (plain, hash) => {
    const first = await sha256(plain);
...
const LoginPage = ({ setPageStack, getUserData, setUserData }) => {
    ...
    const loginAccount = async () => {
        ...
        if ('@' + emailPart[1] != '@gmail.com') { setUserEmailErr("Enter Valid Email Address! "); setLoader(false); return; }

        const result = await Read_From_Online_userdata(getUserEmail);

        if (result.STATUS != 200 || result.DATA.length == 0) {
            setUserEmailErr("There is no Account with this Email");
            setLoader(false);
            return;
        }
        // stored password is bcrypt(sha256(password))
        const match = await verifyPassword(getUserPassword, result.DATA[0].USER_PASS);
        if (match) {
            // keep hashed record but also store original email for later use
            const record = { ...result.DATA[0], originalEmail: getUserEmail };
            setUserData([record, 'login']);
            setLoader(false);
