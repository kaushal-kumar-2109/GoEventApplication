...
const SignupPage = ({ setPageStack, getUserData, setUserData }) => {
    ...
    const createAccount = async () => {
        ...
        setLoader(true);
        setUserData([...getUserData, [getUserEmail, getUserName, getUserNumber, getUserPassword, 'signup']]);

        let res = await Read_From_Online_userdata(getUserEmail);
        if (res.STATUS == 200 && res.DATA.length > 0) {
            setUserEmailErr("This Email Already in use! ");
            setLoader(false)
            return;
        }
        setLoader(false);
